...
const delays = [0.1, 1.6, 2.4, 3.2, 4, 5.5, 5.9, 6.3]
...
const RECENT_VISIT_DURATION_MS =
  process.env.NODE_ENV === "development" ? 1000 * 10 : 1000 * 60 * 60 // hour
        ...
        {visitedRecently ? (
          ...
          <>
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.6, delay: getDelay(1), ease: "easeIn" }}
            >
              Hi there.{" "}
            </motion.span>
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.6, delay: getDelay(2), ease: "easeIn" }}
            >
              This is it.{" "}
            </motion.span>
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.6, delay: getDelay(3), ease: "easeIn" }}
            >
              You've made it.{" "}
            </motion.span>
          </>
